...
import { Canvas as NativeCanvas, ContextAttributes, createCanvas, Image as NativeImage, Path2D, SKRSContext2D } from '@napi-rs/canvas';
...
function resolveImage(resolvable: ImageResolvable) {
...
function resolvePattern(resolvable: PatternResolvable) {
...
export type Transform = ReturnType<SKRSContext2D['getTransform']>;
...
export class Canvas {
	...
	public printImage(image: ImageResolvable, ...args: [number, number]) {
		this.context.drawImage(resolveImage(image), ...args);
		return this;
	}
	...
	public createPattern(
		...
		const pattern = this.context.createPattern(resolvePattern(image), repetition)!;
		if (callback) {
